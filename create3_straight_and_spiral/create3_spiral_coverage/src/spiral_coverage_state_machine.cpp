...
#include "create3_spiral_coverage/spiral_coverage_state_machine.hpp"
#include "tf2/utils.h"

namespace create3_spiral_coverage {

SpiralCoverageStateMachine::SpiralCoverageStateMachine(
...
SpiralCoverageStateMachine::CoverageOutput SpiralCoverageStateMachine::execute(const Behavior::Data& data)
{
...
void SpiralCoverageStateMachine::cancel()
{
    if (m_current_behavior) {
...
void SpiralCoverageStateMachine::select_start_behavior(const Behavior::Data& data)
{
...
void SpiralCoverageStateMachine::select_next_behavior(const Behavior::Data& data)
{
    ...
    switch (m_current_behavior->get_id())
    {
        ...
        case FeedbackMsg::ROTATE:
        {
            ...
            // After rotate, try to move away from the obstacle a little bit and then spiral
            drive_config.max_distance = 0.25; //change this as needed
            drive_config.min_distance = 0.25; //change this as needed
            m_preparing_spiral = true;
        ...
        case FeedbackMsg::SPIRAL:
        {
            ...
            // If failed a spiral, rotate, otherwise keep spiraling
            if (m_behavior_state == State::FAILURE) {
                auto rotate_config = RotateBehavior::Config();
                rotate_config.robot_has_reflexes = m_has_reflexes;
                this->goto_rotate(rotate_config);
...
void SpiralCoverageStateMachine::goto_dock()
{
    m_current_behavior = std::make_unique<DockBehavior>(m_dock_action_client, m_logger);
    m_coverage_output.state = State::RUNNING;
}

void SpiralCoverageStateMachine::goto_drive_straight(const DriveStraightBehavior::Config& config)
{
    m_current_behavior = std::make_shared<DriveStraightBehavior>(config, m_cmd_vel_publisher, m_logger, m_clock);
    m_coverage_output.state = State::RUNNING;
}

void SpiralCoverageStateMachine::goto_rotate(const RotateBehavior::Config& config)
{
    m_current_behavior = std::make_shared<RotateBehavior>(config, m_cmd_vel_publisher, m_logger, m_clock);
    m_coverage_output.state = State::RUNNING;
}

void SpiralCoverageStateMachine::goto_spiral(const SpiralBehavior::Config& config)
{
...
void SpiralCoverageStateMachine::goto_undock()
{
    m_undocking = true;
